...
export default function UpdateScale({ employee, setIsOpenEmployee, isOpenEmployee }) {
  ...
  const scale = scales?.result?.find(scale => (scale.escala.id_escala == employee.id_escala))?.escala
  ...
  useEffect(() => {
    if (isOpenEmployee && employee)
      setForm({
        ...
        usa_dias_especificos: scale?.usa_dias_especificos,
        dias_n_trabalhados_escala_semanal:
          Array.isArray(scale?.dias_n_trabalhados_escala_semanal)
  ...
  const handleDiasChange = (dia) => {
    setForm((prev) => {
      ...
      const dias = Array.isArray(prev.dias_n_trabalhados_escala_semanal)
        ...
        : typeof prev.dias_n_trabalhados_escala_semanal === 'string'
          ? prev.dias_n_trabalhados_escala_semanal.split(',').map(d => d.trim())
          : [];
